...
connecting to `jenkins_docker_cloud_docker_host_uri` (e.g. `tcp://docker-host:2376`) over
**mTLS** with the client cert/key from Vault. Mechanism: **JCasC** (declarative, in
code).

`jenkins_docker_cloud_docker_host_uri` and `jenkins_docker_cloud_agent_image` have no role
...
pre-pulls the agent image (`jenkins_docker_cloud_agent_image`) onto the docker-host itself
...
`jenkins_docker_cloud_agent_network` (also no role default) sets `dockerTemplateBase`'s
